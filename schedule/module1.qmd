...
We now motivate two subjects of probability theory based on experiment
...
**Definition:** An event is a subset of sample space $\Omega$.
...
## De Morgan's Laws (use diagram)

Theorem: for any two events (sets) $A$ and $B$, we have
$$( A\cup B ) ^{c} \, = \, A^{c}\cap B^{c}.$$

**Proof**: To prove two events are equal as above, the general
...
**Probability function** is a function defined on ${\cal B}$ so that
...
Proof of "$A \subset B \, \Rightarrow \, P ( A) \leq P ( B)$:"
...
- By Axiom 2, $P ( B\cap A^{c}) \ge 0$. Thus,
    $$P ( B ) = P ( A ) + P  ( B\cap A^{c}) \ge  P ( A ).$$ This
    completes the proof.
...
Proof of "$P ( A\cup B ) =P ( A ) +P( B ) -P ( A\cap B )$:" 
...
- The mathematics induction is completed now and the formula is
    proved.
...
**Solution**: Introduce two notations for events:
...
**(a) Jane passes at least one test**
...
&=
0.80+0.70-0.60 \\ 
& =  0.90\end{aligned}$$ which is the answer to (a).



## Solution to (b)

**(b) Jane passes at most one test**
...
Continue from the last slide, we have

$$\begin{aligned}
P ( \{ \text{Passes at most one test} \}  )
&= P\left(\left( A\cap B\right) ^{c}\right) \text{ \ \ \ } \\
&=1-P\left( A\cap B\right) \\
&=1-0.60=0.40\end{aligned}$$ which is the required answer.
...
Continue from the last slide, we have $$\begin{aligned}
...
&=1-0.90 \\
&=
 0.10 \text{  from Part (a)}\end{aligned}$$ which is the required
answer.
...
- It is most effective to first introduce some simple events, and
    express other events of interest by these events.
...
## Example without being given a story
...
**(b) Prove the Bonferroni inequality:**
...
**Proof**: First note that $$\bigcap _{i=1}^{n}A_{i} \, = \,  
...
## Refresh the memory of probability axioms
...
**(a)** The sample space consists of all sequences
...
**(b)  When the number of rolls is** $n$, we have algebraic
descriptions:
...
**(c) Verbally interpreting** $E_{1}\cup E_{2}$,
$\bigcup_{n=1}^{\infty} E_{n}$, and
...
Consider the tasks on the next slide
...
**(b) How many outcomes are there in $\Omega$ ?** 
...
**(e) Describe all the outcomes in $A \cap D$**
...
**(a)** If one family is chosen at random, what is the probability
...
**(b)** If one child is randomly chosen, what is the probability that it
